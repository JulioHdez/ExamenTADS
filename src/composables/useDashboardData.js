...
export function useDashboardData() {
  ...
  // Scatter chart data - correlación entre promedio y semestre
  // Mostrar promedio general vs semestre actual
  const scatterData = computed(() => {
    return filteredStudents.value.map(student => ({
      x: parseFloat(student.promedio_general || 0), // Promedio en escala 0-100
      y: parseInt(student.semestre_actual || 0), // Semestre actual
      period: student.semestre_actual,
      name: `${student.nombre} ${student.apellido_paterno}`,
      studentId: student.id_estudiante
    })).filter(point => {
      // Solo incluir puntos con datos válidos
  ...
  const pieChartData = computed(() => {
    ...
    const approved = filteredStudents.value.filter(student => (student.promedio_general || 0) >= 70).length
    const failed = filteredStudents.value.filter(student => (student.promedio_general || 0) < 70 && (student.promedio_general || 0) > 0).length
  ...
  const lineChartData = computed(() => {
    ...
    filteredStudents.value.forEach((student, index) => {
      ...
      if ((student.promedio_general || 0) >= 70) {
