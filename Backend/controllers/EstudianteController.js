...
class EstudianteController extends BaseController {
    ...
    async getAllWithCarreras(req, res) {
        try {
            // Si no se especifica limit, obtener todos los estudiantes
            const { page = 1, limit, orderBy, orderDirection = 'ASC', ...filters } = req.query;
            
            const options = {
                page: parseInt(page),
                limit: limit ? parseInt(limit) : 10000, // Por defecto 10000 para obtener todos
                orderBy,
                orderDirection,
                where: filters
            };
    ...
    async getStats(req, res) {
        try {
            ...
            console.log('Stats desde modelo:', stats);
